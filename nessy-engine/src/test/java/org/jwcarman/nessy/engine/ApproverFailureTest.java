...
/**
 * An approver that breaks instead of answering.
 *
 * <p>A desk can be down as easily as a person can say no, and the call still has to settle: the
 * model is told the call was not made rather than the turn hanging on an approver that will never
  ...
  static void start() {
    ...
    Tool<Letter> send =
        new Tool<>() {
          ...
          public Awaited<ToolResult> execute(ToolCallRequest<Letter> call) {
            throw new AssertionError("the approver never let this run");
          }
        };
    ToolBinding<Letter> binding =
        new ToolBinding<>(
            send,
            request -> {
              throw new IllegalStateException("the approvals desk is down");
            },
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              ...
              assertThat(text)
                  .contains("the approver failed")
                  .contains("the approvals desk is down");
            });
  }
}

...
/**
 * An approved tool whose own code throws.
 *
 * <p>Unlike a tool that reports its own {@link ToolResult#error}, this one never gets the chance
 * to: the exception happens before it can construct one. The call still has to settle, and the
 * message the model reads has to say the honest, weaker thing — it MAY have partially completed,
    ...
    Tool<Letter> send =
        new Tool<>() {
          ...
          public Awaited<ToolResult> execute(ToolCallRequest<Letter> call) {
            throw new IllegalStateException("the mail server refused the connection");
          }
        };
    ToolBinding<Letter> binding =
        new ToolBinding<>(send, Approver.always(), ActionRenderer.byToString());
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              ...
              assertThat(text)
                  .contains("the mail server refused the connection")
                  .contains("it may have partially completed");
            });
  }
}

...
Four contract classes, `org.jwcarman.nessy.tck`, one per SPI seam:
...
field — see `nessy-jdbc`'s five vendor-matrix test classes for that
...
its contract exactly as shown above. `nessy-jdbc` runs the same four

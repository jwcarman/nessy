 ...
 * stores. Not application vocabulary — no user-facing type refers to it, and package-private since
 * every caller ({@link org.jwcarman.nessy.agent.DeliveryWorker}, {@link SubstrateComputations})
 * lives in this same package (durable-dissolves spec §2 reverses the harness-first fix round's
 * widening now that {@code agent.durable} has collapsed back into {@code agent}).
 *
 * <p>{@link Outcome}, {@link Decision}, and {@link ToolResult} live in {@code nessy-api} and stay
 * Jackson-free by design. This codec binds a private, mapper-annotated wire shape instead and
 * hand-translates it to and from the domain types; that translation IS the closed-vocabulary door
 * for the delivery's outcome payload — a {@code Success} outcome whose payload is neither {@link
 * ToolResult} nor {@link Decision} is rejected here, before any write.
 *

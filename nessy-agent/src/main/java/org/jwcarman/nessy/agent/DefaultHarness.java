 ...
 */
public final class DefaultHarness<O> implements Harness<O> {
   ...
   */
  public static <O> DefaultHarness<O> of(
      AgentType type,
      String provider,
      String modelId,
      ObservationRenderer<O> renderer,
  ...
  public static <O> DefaultHarness<O> of(
      ...
      ConcurrentMap<AgentId, Observation> openSegments,
      ExecutorService ownedExecutor) {
    // Constructed here, not shared across separate DefaultHarness.of(...) calls (continuum-adoption
    // spec
    ...
    // published to other threads (no-`this`-escape). Registering after
    // `new DefaultHarness<>(...)` returns guarantees safe publication.

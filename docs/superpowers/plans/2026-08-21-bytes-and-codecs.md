...
**Files:** rename `StoredAgentStateStore`→`SubstrateAgentStateStore`, `StoredMemory`→`SubstrateMemory`, `StoredBacklog`→`SubstrateBacklog`, `StoredComputations`→`SubstrateComputations` (nessy-agent) and `StoredIntentStore`→`SubstrateIntentStore` (nessy-intent), including test classes. Each gains the optional `Codec<T>` constructor parameter (spec §3): state — `Codec<Phase>` defaulting to the StateCodec binding; memory — `Codec<Message>` defaulting to MessageCodec's; computations — internal document codec stays internal (its payload vocabulary is closed; no public codec param — record WHY in javadoc); intent — `Codec<T>` defaulting to `Codec.json(mapper, vocabulary)`; backlog — see Task 5 (generic). Task 1's `getBytes` shims dissolve into the codecs. Remove remaining String-signature indirection so recipes speak `Codec<T>` → `byte[]` → substrate.

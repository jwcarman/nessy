...
> Computations are now a substrate recipe (`kind=computation`), the outbox is `batch([flip,
> enqueue])` over the substrate, and adapters implement `Substrate` — never this spec's SPI.

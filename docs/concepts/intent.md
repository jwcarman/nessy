...
```java
IntentStore<Intent> store = new JdbcIntentStore<>(dataSource, TYPE, Intent.class, mapper);

config.tool(IntentTool.freeform(store));
```

A store is scoped to one agent type and keeps the latest declaration per
agent id. The model calls the tool, the declaration replaces whatever that
agent declared before, and `store.latest(agentId)` is what an approver
reads.
...
IntentStore<OpsIntent> store = new JdbcIntentStore<>(dataSource, TYPE, OpsIntent.class, mapper);
...
```java
IntentEnricher<OpsIntent> enricher = new IntentEnricher<>(store, mapper);
```
...
that an approver still makes; see [Authorization](authorization.md). A
model that has learned to declare something harmless and then ask for
something else is exactly the case the comparison exists to catch, which
only works if somebody does the comparing.

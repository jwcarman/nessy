...
server. `chat-web`, `dispatcher`, and `order-desk` need a provider key and
Docker, for Postgres and (for `order-desk`) RabbitMQ. `night-watchman`
needs only a provider key.

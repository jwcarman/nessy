...
/**
 * The full TCK, all eight contracts, run against a real SQL Server — plus the dialect-resolution
 * pin (design §6). One container for the whole class (nested contracts share it, each truncating
 * its own table between tests) rather than seven, the same efficiency trade the vendor matrix needs
 * five times over. Requires Docker; tagged {@code container} so the offline default build never
 * needs it.
 *
 * <p>Image pinned to {@code mcr.microsoft.com/mssql/server:2022-latest} — the exact tag Task 2's
 * live schema verification ran against (see that task's report); this matrix reuses those findings
 * rather than re-discovering them against a different point release. {@link
 * MSSQLServerContainer#acceptLicense()} accepts the EULA the image otherwise refuses to start
 * without — a Testcontainers-module requirement, not a project-specific choice.
 */
@Testcontainers
@Tag("container")
@Tag("vendor")
class SqlServerStoreTckTest {
  ...
  static void nessy_jdbc_test_points_a_data_source_at_the_container() {
    dataSource =
        new DriverManagerDataSource(MSSQL.getJdbcUrl(), MSSQL.getUsername(), MSSQL.getPassword());
  }

  @Test
  void the_resolver_picks_sql_server_against_the_real_container() throws SQLException {
    try (Connection connection = dataSource.getConnection()) {
      assertThat(JdbcDialect.resolve(connection.getMetaData())).isEqualTo(JdbcDialect.SQLSERVER);
    }
  }
  ...
  private static final class DriverManagerDataSource implements DataSource {
    ...
    public PrintWriter getLogWriter() {
      throw new UnsupportedOperationException("not used by SqlServerStoreTckTest");
    }

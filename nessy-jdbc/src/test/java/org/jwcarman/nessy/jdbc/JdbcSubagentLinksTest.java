...
/**
 * The TCK run against a real Postgres, plus a JDBC-specific pin the in-memory registry has no
...
@Testcontainers
@Tag("container")
class JdbcSubagentLinksTest extends SubagentLinksContract {
  ...
  private void truncateLinksTable() {
    try (Connection connection = dataSource.getConnection();
        Statement statement = connection.createStatement()) {
      statement.execute("TRUNCATE nessy_subagent_links");
    } catch (SQLException e) {
      throw new IllegalStateException("failed to truncate nessy_subagent_links between tests", e);
    }
  }
  ...
  void the_schema_bootstrap_is_idempotent() {
    assertThatCode(
            () -> {
              JdbcSubagentLinks.create(dataSource);
              JdbcSubagentLinks.create(dataSource);
            })
  ...
  private static final class DriverManagerDataSource implements DataSource {
    ...
    public PrintWriter getLogWriter() {
      throw new UnsupportedOperationException("not used by JdbcSubagentLinksTest");
    }

...
/**
 * The full TCK, all eight contracts, run against a real MySQL — plus the dialect-resolution pin
 ...
 * needs it.
 *
 * <p>Image pinned to {@code mysql:8.0} — the exact tag Task 2's live schema/SQLState verification
 * ran against (see that task's report); this matrix reuses those findings rather than
 * re-discovering them against a different point release.
 */
@Testcontainers
@Tag("container")
@Tag("vendor")
class MySqlStoreTckTest {
  ...
  /**
   * The sniff's easy half: MySQL's own driver reports the product name {@code "MySQL"} and a
   * version string that does not mention MariaDB, so the resolver should land on {@link
   * JdbcDialect#MYSQL} directly — see {@link MariaDbStoreTckTest} for the tripwire half.
   */
  @Test
  void the_resolver_picks_mysql_against_the_real_container() throws SQLException {
    try (Connection connection = dataSource.getConnection()) {
      assertThat(JdbcDialect.resolve(connection.getMetaData())).isEqualTo(JdbcDialect.MYSQL);
    }
  }
  ...
  private static final class DriverManagerDataSource implements DataSource {
    ...
    public PrintWriter getLogWriter() {
      throw new UnsupportedOperationException("not used by MySqlStoreTckTest");
    }

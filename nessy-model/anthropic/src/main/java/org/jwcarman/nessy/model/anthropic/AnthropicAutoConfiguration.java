 ...
 * as {@code anthropic.api-key}) contributes an {@link AnthropicModelProvider} bean built from that
 * key alone — the same {@code create(c -> c.apiKey(key))} an application would write, not {@link
 * AnthropicModelProvider#fromEnv()}, so the key Boot saw is the key that gets built and no other
...
public class AnthropicAutoConfiguration {
  ...
  public ModelProvider anthropicModelProvider(@Value("${anthropic.api-key}") String apiKey) {

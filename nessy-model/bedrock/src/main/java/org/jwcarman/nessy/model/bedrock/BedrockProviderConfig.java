...
/**
 * What {@link BedrockModelProvider#create(BedrockProviderCustomizer)} hands a customizer: a CONFIG,
...
public final class BedrockProviderConfig {
   ...
   * seam-integrity rule the sibling provider modules established: the choice this config makes from
   * the environment is the choice that gets built, not a second, independent read underneath it.
   *
   * <p>Only a flag is set here; nothing is read yet. {@link #build()} applies it, layering any
   * explicit {@link #region(Region)} set on <em>this</em> config on top so it wins over either
   * environment variable.
   *
   * @throws IllegalStateException at {@link #build()} time if neither an explicit region nor either
  ...
  /**
   * Escape hatch: supply a fully preconfigured {@link BedrockRuntimeAsyncClient} instead of {@code
   * region}/{@code credentialsProvider}.
   *
   * <p><b>Ownership stays with the caller.</b> {@link BedrockModelProvider#close()} closes only a
   * client this config constructed itself from {@code region}/{@code credentialsProvider}/{@code
   * fromEnv()} — a client supplied here is never closed by the provider, since it was never opened
   * by the provider either. Close {@code client} yourself, on whatever lifecycle you built it
   * against.
   */
  ...
  /**
   * Turns this config into the {@link BedrockModelProvider} it describes — the factory's own step,
   * never a public {@code build()} (design of record 2026-08-16 §1). Reached only from {@link
   * BedrockModelProvider#create(BedrockProviderCustomizer)}, once {@code customize} has returned.
   */
  BedrockModelProvider build() {
   ...
   * {@link BridgeIterator} this method hands to {@link BedrockStream} pulls from that same queue on
   * the caller's thread. The queue also carries the stream's own end-of-stream signals — {@link
   * #DONE} on success, a {@link StreamFailure} on error — so a caller iterating the resulting
   * {@link BedrockStream} sees a normal end of iteration or a thrown exception, never a silently
   * truncated stream.
   *
   * <p>The queue is deliberately unbounded ({@link LinkedBlockingQueue}'s no-capacity constructor):
   * the producer side runs on the SDK's own Netty event-loop thread, and blocking an event loop on
   * a full queue would stall every other request multiplexed over that loop — far worse than
   * letting one turn's events buffer in memory. {@code maxTokens} already bounds how much one turn
   * can produce, so the buffer is bounded in practice even though the queue itself is not.
   *
   ...
   * never close resources it did not open, the same convention a caller-supplied {@code DataSource}
   * or {@code ExecutorService} follows elsewhere. Package-private rather than {@code private} so
   * the ownership branch is directly testable without a real, network-capable SDK client on either
   * side (see {@code BedrockModelProviderTest$CloseOwnership}).
   */
  ...
  private static final class QueueingVisitor implements ConverseStreamResponseHandler.Visitor {
    ...
    /**
     * The queue is unbounded (see {@link #wrap}'s javadoc), so {@link BlockingQueue#add(Object)}
     * never actually blocks here — it says that plainly, unlike a {@code put} that would need
     * interrupt handling it can never hit in practice.
     */
   ...
   *
   * <p>Constructed with the stream's already-taken first item ({@link #wrap}'s pump-priming read)
   * so that item is not lost — it becomes this iterator's initial lookahead (or marks it already
   * finished, if the very first item was {@link #DONE}) rather than being re-fetched from the
   * queue.
   */

 ...
 * one-line notice on {@link System#err} when that variable is unset or unrecognized; neither
 * present fails fast, naming exactly the three variables checked. Each provider is built the same
 * way its own module builds one from an explicit key — {@code
 * Provider.builder().apiKey(key).build()} — mirroring {@link
 * AnthropicModelProvider.Builder#apiKey(String)} and {@link
    ...
    if (!ANTHROPIC_CHOICE.equalsIgnoreCase(preference)) {
      System.err.println(
          "both "
              ...
              + " and "
              + OPENAI_API_KEY_ENV_VAR
              + " are set; defaulting to Anthropic (set "
              + NESSY_PROVIDER_ENV_VAR
              + "=openai to choose OpenAI instead)");
    }
    return anthropic(anthropicKey);
  }

...
#
# It DESTROYS the watchman schema on every run. That is deliberate -- a
# soak that inherits state measures the state, not the code.
# ---------------------------------------------------------------------------
set -uo pipefail

ROUNDS_TARGET="${1:-8}"
MODULE="nessy-examples/watchman"
SCHEMA="watchman"
PG_CONTAINER="${PG_CONTAINER:-watchman-postgres}"
LOG_DIR="${HOME}/.local/state/nessy"
LOG="${LOG_DIR}/watchman.log"
JAR="${MODULE}/target/nessy-example-watchman-0.1.0-SNAPSHOT.jar"
...
pkill -9 -f "watchman/target" 2>/dev/null

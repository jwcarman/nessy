...
public interface ConversationStore {
   ...
   * atomically bumping to {@code version()+1} and deleting the drained inbox entries — one atomic
   * act, and nothing else. Returns the state with the bumped version (the caller's new read-base).
   * Readers observe this act atomically too: {@link #load} never sees the state and the inbox from
   * one generation mixed with the other from a different one — a concurrent reader sees either
   * every effect of this save or none of them.
   *

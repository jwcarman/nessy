...
public abstract class ParksContract {
  ...
  /** A fresh, empty registry — called once per test, never reused across tests. */
  ...
  @Test
  void a_registered_entry_survives_resolution_it_is_never_deleted_by_this_registry() {
    ConversationId id = ConversationId.generate();
    ParkToken token = ParkToken.generate();
    Park park = new Park(id, token, toolCall("c1"));
    parks().park(park);
    ...
    // the same keep-forever posture the retired nessy_token table already had.
    assertThat(parks().find(token)).contains(park);
    assertThat(parks().find(token)).contains(park);
  }
}

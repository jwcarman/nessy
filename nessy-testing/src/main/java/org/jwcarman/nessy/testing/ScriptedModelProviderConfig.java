...
/**
 * What {@link ScriptedModelProvider#script(ScriptedModelProviderCustomizer)} hands a customizer: a
 * CONFIG, not a builder (design of record 2026-08-16 §1) — fluent setters, no public {@code
 * build()}. Scripts one turn at a time: each event-adding call ({@link #text}, {@link #thinking},
 * etc.) appends to the turn under construction, and {@link #endTurn()}/{@link #endWithToolUse()}
 * closes it and starts the next.
 */
public final class ScriptedModelProviderConfig {
  ...
  public ScriptedModelProviderConfig text(String text) {
    current.add(new ModelEvent.TextChunk(text));
    return this;
  }

  public ScriptedModelProviderConfig thinking(String text) {
    current.add(new ModelEvent.ThinkingChunk(text));
    return this;
  }

  public ScriptedModelProviderConfig thinkingSigned(String signature) {
    current.add(new ModelEvent.ThinkingSigned(signature));
    return this;
  }

  public ScriptedModelProviderConfig redactedThinking(String data) {
    current.add(new ModelEvent.RedactedThinkingEmitted(data));
    return this;
  }

  public ScriptedModelProviderConfig toolUse(String id, String name, ObjectNode arguments) {
    current.add(new ModelEvent.ToolUseEmitted(new ToolCall(id, name, arguments)));
    return this;
  }

  public ScriptedModelProviderConfig toolUseSigned(
  ...
  public ScriptedModelProviderConfig endTurn() {
    return end(StopReason.END_TURN, Usage.zero());
  }

  public ScriptedModelProviderConfig endTurn(Usage usage) {
    return end(StopReason.END_TURN, usage);
  }

  public ScriptedModelProviderConfig endWithToolUse() {
    return end(StopReason.TOOL_USE, Usage.zero());
  }

  private ScriptedModelProviderConfig end(StopReason reason, Usage usage) {
  ...
  /**
   * Turns this config into the {@link ScriptedModelProvider} it describes — the factory's own step,
   * never a public {@code build()} (design of record 2026-08-16 §1). Reached only from {@link
   * ScriptedModelProvider#script(ScriptedModelProviderCustomizer)}, once {@code customize} has
   * returned.
   *
   * @throws IllegalStateException if the last turn was never ended
   */
  ScriptedModelProvider build() {
    if (!current.isEmpty()) {
